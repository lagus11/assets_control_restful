            ...
            UserSchema.findOne({
              ...
              try {
                if (err) {
                  ...
                  res.status(403).json({
                    status: 'USUARIO Y/O CONTRASENA INCORRECTA'
                  });
                } else {
...
router.post("/refreshToken", /*#__PURE__*/function () {
  var _ref2 = _asyncToGenerator( /*#__PURE__*/_regeneratorRuntime().mark(function _callee2(req, res) {
    var refreshTokenCookie, _jwt$verify, user, newAccessToken, TokenVerificationErrors;
      ...
      while (1) {
        switch (_context2.prev = _context2.next) {
          case 0:
            ...
            throw SyntaxError("Not exist RefreshToken");
          case 4:
            _jwt$verify = jwt.verify(refreshTokenCookie, process.env.secretRefreshToken), user = _jwt$verify.user;
            refreshTokens = refreshTokens.filter(function (token) {
              return token !== refreshTokenCookie;
            });
            console.log(user);
            newAccessToken = generateToken(user); //const newRefreshToken = generateRefreshToken(user, res); <---- genera nuevo refresh 1h, causa siempre este activa
            //refreshTokens.push(newRefreshToken);
            //return res.status(200).json(newAccessToken);
            return _context2.abrupt("return", res.tatus(200).json());
          case 11:
            _context2.prev = 11;
            ...
            if (!(_context2.t0.message === 'jwt expired')) {
              _context2.next = 16;
              break;
            }
            return _context2.abrupt("return", res.status(401).json(TokenVerificationErrors[_context2.t0.message]));
          case 16:
            return _context2.abrupt("return", res.status(403).json(TokenVerificationErrors[_context2.t0.message]));
          case 17:
          case "end":
...
router.post("/logout", verifyToken, function (req, res) {
  res.status(200).json({
    status: "You logged out successfully"
  });
});
