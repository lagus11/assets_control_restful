const jwt = require("jsonwebtoken");
const jwt_decode = require('jwt-decode');

function validateTokenMicrosoft(req, res, next) {

    let { token } = req.body; //obtengo token del body
    // clave publica del token de azure microsoft "ocupa buscarse en cada cuenta"
    const publicKey = `-----BEGIN CERTIFICATE-----\r\\r\n-----END CERTIFICATE-----`;

    if (token) { 
        token = token.split(" ")[1]; //separo el token bearer
        //verifico que el token sea de azure
        jwt.verify(token, publicKey, {algorithms: ['RS256'] }, (err, usuario) =>{
          if(err){
            console.log(err);
            return res.status(404).json({status : "TokenMicrosoft is not valid"});
          }
          const email = jwt_decode(token).preferred_username; //obtengo correo del payload
          req.email = email; //paso al req el correo para usarlo en busqueda en bd
          next();
        });
      }else {
        res.status(401).json({status: "You are not authenticated"});
      }
}

module.exports = validateTokenMicrosoft;